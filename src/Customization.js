...
export default class ColorSelectionScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.add
      .image(config.width / 2 - arrowOffset, config.height / 2, "left")
      .setInteractive()
      .setScale(arrowScale)
      .on("pointerdown", () => this.changeShip(-1));

    this.add
      .image(config.width / 2 + arrowOffset, config.height / 2, "right")
      .setInteractive()
      .setScale(arrowScale)
      .on("pointerdown", () => this.changeShip(1));

    this.confirmButton = this.add
      .text(config.width / 2, config.height - 100, "Confirm", {
        stroke: "#111",
        strokeThickness: 4,
        fontSize: 20,
        fill: "#FFF",
